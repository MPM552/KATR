...
# CGI-bin script to look words up in my Bartang word list.
# This script displays words, and can call showBartang.cgi for KATR-generate
# details on any word.
...
# parameters
	...
	my $dictionaryFile = "$wordDir/bartang.vocab";
	my $cgiDir = "http://www.cs.uky.edu/~raphael/linguistics";
	my $formFile = "$cgiDir/bartangDict.cgi";
	my $Debug = 0;
...
sub printForm {
	...
	A word or phrase or fragment to search (Bartang or English): 
...
sub printNicely {
	...
	# items without parameters are listed later.
	my %codeToKATR = (
		inf => 'infinitive stem',
		object => 'sample direct object',
		past => 'past stem',
		past1sg => 'past stem for 1st person singular',
	...
	push @parts, $line;
	# items without paramaters are listed here.
	for my $part (@parts) {
		...
		} elsif ($part =~ s/^emptyImpSuffix:\s*//) {
			push @toPrint, "<span class=\"grammar\">imperative has no -t;</span>";
		} elsif ($part =~ s/^note:\s+//) {
...
sub doIt {
	...
	print header(-charset=>'UTF-8', -expires=>'-1d'),
		start_html(
			-encoding=>'UTF-8',
			-title=>'Bartang dictionary lookup',
	...
	print "" . br() . hr() . br();
	if ($searchWord =~ /^\s*$/) {
		print "Type in a word, either in Bartang or English, followed by the
		enter key.";
	} else {
...
# other useful files
# showBartang.cgi    calls KATR
# ~/links/r/bartang.katr
# ~/links/r/bartang.vocab
# ~/links/r/bartang.true
# vim:nospell:
